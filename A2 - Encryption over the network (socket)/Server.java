...
public class Server {
    ...
    private static String INPUTFILE = "clientciphertext.txt";
    ...
    public static void main (String [] args) {
        ...
        if (args.length == 2) {
            if (args[1].equals("debug")) {
                DEBUG = true;
                System.out.println("Usage: Server DEBUG mode active!");
            }
        }

        // Check for existence of arguments and make sure port number is valid
        if (args.length == 0) {
        ...
        try {
            if (Integer.parseInt(args[0]) < 1023 || Integer.parseInt(args[0]) > 65535) {
                System.out.println("Error: java Server port not valid or out of range!#");
                return;
            } else {
                port = Integer.parseInt(args[0]);
                if (DEBUG) System.out.println("The port number is: " + port);
            }
	...
	while (!shutdown) {
	    ...
	    try {
            ...
            System.out.println ("Client on machine " + client.getInetAddress().getHostAddress() +
                    " has connected on port " + client.getLocalPort() + "");
